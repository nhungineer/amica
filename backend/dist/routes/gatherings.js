...
router.post("/", auth_1.requireAuth, async (req, res) => {
    try {
        const { title, location, timezone, timeOptions, rsvpDeadline, } = req.body;
        // Get organizerId from authenticated user
        const organizerId = req.user.userId;
        const gathering = await db_1.prisma.gathering.create({
            data: {
                organizerId,
                title,
                location,
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        const gathering = await db_1.prisma.gathering.update({
            where: { id },
            data: {
                ...(status && { status }),
