...
const RESEND_FROM_EMAIL = process.env.RESEND_FROM_EMAIL || "Amica <onboarding@resend.dev>";
...
router.post("/send-magic-link", async (req, res) => {
    try {
        ...
        await db_1.prisma.loginToken.create({
            data: {
                token,
                userId: user.id,
                email: user.email,
        ...
        const magicLink = `${FRONTEND_URL}/auth/verify?token=${token}${redirectTo ? `&redirect=${encodeURIComponent(redirectTo)}` : ""}`;
...
router.get("/verify-token", async (req, res) => {
    try {
        ...
        const loginToken = await db_1.prisma.loginToken.findUnique({
            where: { token },
            include: { user: true },
        });
        // Validation checks
        ...
        // Mark token as used
        await db_1.prisma.loginToken.update({
            where: { token },
